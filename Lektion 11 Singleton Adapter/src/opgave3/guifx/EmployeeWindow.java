...
import opgave3.application.service.Service;
...
public class EmployeeWindow extends Stage {
    ...
    private void initContent(GridPane pane) {
        ...
        cbbCompany.getItems().addAll(Service.getService().getCompanies());
    ...
    private void okAction() {
        ...
        if (employee != null) {
            Service.getService().updateEmployee(employee, name, wage);
            if (companyIsSelected) {
                Service.getService().updateCompanyOfEmployee(employee, newCompany);
            }
            else {
                Service.getService().clearCompanyOfEmployee(employee);
            }
        } else {
            if (companyIsSelected) {
                Service.getService().createEmployee(name, wage, newCompany);
            } else {
                Service.getService().createEmployee(name, wage);
            }
        }
